...
class Admin extends BaseController {
	...
	public function register() {
		...
		$this->settings->add_pages($this->pages)->with_sub_page(esc_html('Dashboard', 'zephyr-project-manager'))->add_sub_pages($this->subpages)->register();
	...
	public function addShortcodes() {
		$shortcodes = [
			...
			'zephyr_new_project_button' => array($this, 'newProjectModalShortcode'),
	...
	public function handleZephyrAccess() {
		if (!apply_filters('zpm_has_zephyr_access', true)) {
			wp_die(esc_html("Sorry, you do not have access to this page", 'zephyr-project-manager'));
			exit();
		}
	}
...
?>
	...
	?>
		<div id="zpm-calendar__filter" class="zpm-shortcode-calendar-filters" style="<?php echo !$a['filters'] ? 'display: none;' : ''; ?>">
			<div class="zpm-calendar__filter-section">
				<label class="zpm_label"><?php esc_html_e('Project', 'zephyr-project-manager'); ?></label>
			...
			<?php if (current_user_can('zpm_view_tasks') || !Utillities::is_zephyr_role($userId)) : ?>
				<div class="zpm-calendar__filter-section">
					<label class="zpm_label"><?php esc_html_e('Team', 'zephyr-project-manager'); ?></label>
				...
				<div class="zpm-calendar__filter-section">
					<label class="zpm_label"><?php esc_html_e('Assignee', 'zephyr-project-manager'); ?></label>
			...
			<div class="zpm-calendar__filter-section">
				<label class="zpm_label"><?php esc_html_e('Status', 'zephyr-project-manager'); ?></label>
				<select id="zpm-calendar__filter-completed" class="zpm-select">
					<option value="all"><?php esc_html_e('All', 'zephyr-project-manager'); ?></option>
					<option value="0"><?php esc_html_e('Incompleted', 'zephyr-project-manager'); ?></option>
					<?php foreach ($statuses as $slug => $status) : ?>
						<?php $isCompletedStatus = in_array($slug, ['completed', 'complete']); ?>
						<option value="<?php echo $isCompletedStatus ? '1' : $slug ?>" <?php echo $isCompletedStatus && $a['completed'] == '1' ?>><?php echo esc_html($status['name']); ?></option>
					<?php endforeach; ?>
				</select>
			</div>
		</div>
	...
	function chartShortcodeItem($projectID, $a) {
		...
		<div class="zpm-shortcode-chart-container">
			<canvas id="zpm-shortcode-chart-<?php echo esc_attr($projectID . '-' . Utillities::generate_random_number()); ?>" data-completed="<?php echo esc_attr($stats['completed']); ?>" data-pending="<?php echo esc_attr($stats['pending']); ?>" data-overdue="<?php echo esc_attr($stats['overdue']); ?>" width="400" height="200" data-zpm-chart-shortcode data-project-id="<?php echo esc_attr($projectID); ?>" class="zpm-shortcode-chart"></canvas>
		</div>
		<?php
	}
	...
	?>
		<div class="zpm-shortcode__file-uploader">
			<button class="zpm-shortcode__file-upload-btn" data-project="<?php echo esc_attr($a['project']); ?>" data-task="<?php echo esc_attr($a['task']); ?>"><?php esc_html_e('Upload File', 'zephyr-project-manager'); ?></button>
		</div>
		<?php
	...
	function project_shortcode($atts) {
		...
		if ($a['id'] == 'all' || $a['id'] == '-1') {
			$projects = Projects::get_projects(null, null, null, $a['public']);
			...
			foreach ($projects as $project) {
				echo Projects::shortcodeHtml($project, $a);
			}
		} else {
						...
						<div class="zpm-task-shortcode <?php echo $task->completed == '1' ? 'zpm-task-completed' : ''; ?>" data-task-id="<?php echo esc_attr($task->id); ?>" data-edit-task="<?php echo esc_attr($editTask); ?>">
							<div class="zpm-task-card <?php echo esc_attr($classes); ?>">
								...
								<div class="zpm-task-card-header">
									<small class="zpm-task-card-type"><?php esc_html_e('Task', 'zephyr-project-manager'); ?></small>
						...
						<div class="zpm-task-card-header">
							...
							<div class="zpm-task-card__title">
								...
								<!-- <?php if ($task->completed == '1') : ?>
									<span class="zpm-task-card__completed_label"><?php esc_html_e('Completed', 'zephyr-project-manager'); ?></span>
								<?php endif; ?> -->
							</div>
	...
	public function activityShortcode($atts) {
		...
		ob_start();
		if (!$activities) : ?>
			<div class="zpm_no_results_message"><?php esc_html_e('There is no activity yet. Once there is, the activities will be displayed here.', 'zephyr-project-manager'); ?></div>
		<?php else : ?>
			<?php echo $activities; ?>
		<?php endif; ?>
	<?php
	...
	?>
		<div id="zpm-panel-user-overview" class="zpm-panel zpm-panel-12 zpm-user-overview__shortcode">
			...
			<h4 class="zpm_panel_title"><?php esc_html_e('User Overview', 'zephyr-project-manager'); ?></h4>
			...
			<div id="zpm-project-stat-overview" class="zpm-user-overview-stats">
				<span class="zpm-project-stat">
					...
					<span class="zpm-project-stat-label"><?php esc_html_e('Projects', 'zephyr-project-manager'); ?></span>
					<a class="zpm_button zpm_button_inverted zpm-dashboard__button" href="<?php echo esc_url(Utillities::get_frontend_url('action=projects&user=' . $userId)); ?>"><?php esc_html_e('View All', 'zephyr-project-manager') ?></a>
				</span>
				<span class="zpm-project-stat">
					<span id="zpm_project_stats_total" class="zpm-project-stat-value"><?php echo sizeof($user_tasks); ?></span>
					<span class="zpm-project-stat-label"><?php esc_html_e('Tasks', 'zephyr-project-manager'); ?></span>
					<a class="zpm_button zpm_button_inverted zpm-dashboard__button" href="<?php echo esc_url(Utillities::get_frontend_url('action=tasks&user=' . $userId)); ?>"><?php esc_html_e('View All', 'zephyr-project-manager') ?></a>
				</span>
				<span class="zpm-project-stat">
					<span class="zpm-project-stat-value medium"><?php echo sizeof($user_pending_tasks); ?></span>
					<span class="zpm-project-stat-label"><?php esc_html_e('Pending Tasks', 'zephyr-project-manager'); ?></span>
					<a class="zpm_button zpm_button_inverted zpm-dashboard__button" href="<?php echo esc_url(Utillities::get_frontend_url('action=tasks&user=' . $userId . '&status=pending')); ?>"><?php esc_html_e('View All', 'zephyr-project-manager') ?></a>
				</span>
				<span class="zpm-project-stat">
					<span class="zpm-project-stat-value good"><?php echo sizeof($user_completed_tasks); ?></span>
					<span class="zpm-project-stat-label"><?php esc_html_e('Completed Tasks', 'zephyr-project-manager'); ?></span>
					<a class="zpm_button zpm_button_inverted zpm-dashboard__button" href="<?php echo esc_url(Utillities::get_frontend_url('action=tasks&user=' . $userId . '&completed=true')); ?>"><?php esc_html_e('View All', 'zephyr-project-manager') ?></a>
				</span>
			</div>
		</div>
	<?php
	...
	public function newTaskModalShortcode($atts) {
		...
		<div class="zpm-shortcode__new-task-btn-wrap">
			<button class="zpm-shortcode__new-task-btn" data-trigger="new-task-modal"
				...
				<?php endforeach; ?>>
				<?php esc_html_e('New Task', 'zephyr-project-manager'); ?>
			</button>
		</div>
	<?php
		$html = ob_get_clean();
	...
	public function newProjectModalShortcode($atts) {
		$a = shortcode_atts([
			'name' => 'yes',
			'description' => 'yes',
			'project' => 'yes',
		...
		<div class="zpm-shortcode__new-task-btn-wrap">
			<button class="zpm-shortcode__new-project-btn" data-trigger="new-project-modal"
				<?php foreach ($a as $k => $v): ?>
					<?php echo 'data-' . esc_attr($k) . '="' . esc_attr($v) . '"'; ?>
				<?php endforeach; ?>>
				<?php esc_html_e('New Project', 'zephyr-project-manager'); ?>
			</button>
		</div>
	<?php
		$html = ob_get_clean();
	...
	public static function get_pages() {
		...
		$pages = [[
			/* translators: Project manager page title. %s: Company name */
			'page_title' => sprintf(esc_html('%s Project Manager', 'zephyr-project-manager'), zpm_get_company_name()),
			/* translators: Project manager page title. %s: Company name */
			'menu_title' => sprintf(esc_html('%s Project Manager', 'zephyr-project-manager'), zpm_get_company_name()),
	...
	public function render_dashboard_widget() {
		...
		<div id="zpm_project_overview">
			<span class="zpm_project_stat_section">
				...
				<span class="zpm_project_stat_title"><?php esc_html_e('Projects', 'zephyr-project-manager'); ?></span>
			</span>
			<span class="zpm_project_stat_section">
				<span class="zpm_project_stat_status"><?php echo esc_html($completed_projects); ?></span>
				<span class="zpm_project_stat_title"><?php esc_html_e('Completed', 'zephyr-project-manager'); ?></span>
			</span>
			<span class="zpm_project_stat_section">
				<span class="zpm_project_stat_status"><?php echo esc_html($pending_projects); ?></span>
				<span class="zpm_project_stat_title"><?php esc_html_e('Pending', 'zephyr-project-manager'); ?></span>
			</span>
		</div>
		...
		<div class="zpm_dashboard_projects">
			...
			<div class="zpm-dashboard-widget-buttons">
				<a href="<?php echo esc_url(admin_url('/admin.php?page=zephyr_project_manager_projects')); ?>" class="zpm_button"><?php esc_html_e('View All Projects', 'zephyr-project-manager'); ?></a>
			</div>
		</div>
	<?php
	}
	...
	public function render_dashboard_tasks_widget() {
		...
		<?php if (Tasks::get_task_count() <= 0) : ?>
			<p><?php esc_html_e('There are no tasks to view at the moment.', 'zephyr-project-manager'); ?></p>
			<?php return; ?>
		<?php endif; ?>
		<h3 class="zpm_dashboard_heading"><?php esc_html_e('My Tasks Due This Weeks', 'zephyr-project-manager'); ?>:</h3>
		...
		<?php if (empty($week_tasks)) : ?>
			<p><?php esc_html_e('You have no tasks due this week', 'zephyr-project-manager'); ?>.</p>
		<?php endif; ?>

		<h3 class="zpm_dashboard_heading"><?php esc_html_e('My Overdue Tasks', 'zephyr-project-manager'); ?>:</h3>
		<ul class="zpm_admin_list">
		...
		<?php if (empty($overdue_tasks)) : ?>
			<p><?php esc_html_e('You have no overdue tasks.', 'zephyr-project-manager'); ?></p>
		<?php endif; ?>

		<div class="zpm-dashboard-widget-buttons">
			<a href="<?php echo esc_url(admin_url('/admin.php?page=zephyr_project_manager_tasks')); ?>" class="zpm_button"><?php esc_html_e('See All Tasks', 'zephyr-project-manager'); ?></a>
		</div>
	...
	?>
		<div class="zpm_update_notice zpm_admin_notice update notice">
			<?php /* translators: Message prompting the user to get started with Zephyr Project Manager. %1$s: Opening anchor tag for admin URL, %2$s: Closing anchor tag */ ?>
			<p><?php printf(esc_html('Get started with Zephyr Project Manager now from %1$s here %2$s', 'zephyr-project-manager'), '<a href="' . esc_url(admin_url('/admin.php?page=zephyr_project_manager')) . '" class="zpm_link">', '</a>'); ?></p>
		</div>
	<?php
	}
	...
	?>
		<div class="zpm_update_notice zpm_admin_notice update notice">
			...
			<p><?php esc_html_e('Thanks for using Zephyr Project Manager. If you enjoy it, could you please consider leaving a review? It would really mean the world to me!', 'zephyr-project-manager'); ?></p>
			<button class="zpm_button"><a href="https://wordpress.org/support/plugin/zephyr-project-manager/reviews/" target="_blank"><?php esc_html_e('Leave a Review', 'zephyr-project-manager'); ?></a></button>
		</div>
	<?php
	}
	...
	?>
		<div class="zpm_update_notice zpm_admin_notice update notice">
			...
			<h4 class="zpm_notice_heading"><?php esc_html_e('Welcome to Zephyr Project Manager', 'zephyr-project-manager'); ?></h4>
			<p class="zpm_panel_description">
				<?php esc_html_e('Thanks for using Zephyr Project Manager. If you experience any problems or have any feature requests, I would be more than happy to add them. Please contact me at dylanjkotze@gmail.com for any queries.', 'zephyr-project-manager') ?>
			</p>
		</div>
<?php
	}
	...
	public function newUserCreated($userId) {
		...
		if (!Utillities::is_zephyr_role($userId)) {
			if ($settings['auto_allow_new_users']) {
