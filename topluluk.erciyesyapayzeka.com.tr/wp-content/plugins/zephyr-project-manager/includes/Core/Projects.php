...
class Projects {
	...
	public static function get_projects($limit = null, $args = null, $filters = null, $public = false, $userId = null) {
		...
		return array_filter($projects, function($project) use ($public, $userId) {
			...
			$hasAccess = Projects::has_project_access($project);
	...
	public static function get_incomplete_projects() {
		global $wpdb;
		$table_name = ZPM_PROJECTS_TABLE;
		$projects = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$table_name} WHERE completed = %d ORDER BY id DESC", 0));
		foreach ($projects as $project) {
	...
	public static function project_created_by($project_id) {
		...
		if (is_object($user)) {
			/* translators: Display format for task creation timestamp. 1: User display name, 2: Date (today or specific), 3: Time */
			$return = ($today->format('Y-m-d') == $created_on->format('Y-m-d')) ? sprintf(esc_html('Created by %1$s at %2$s today', 'zephyr-project-manager'), $user->display_name, $created_on->format('H:i')) : sprintf(esc_html('Created by %1$s on %2$s at %3$s', 'zephyr-project-manager'), $user->display_name, $created_on->format('d M'), $created_on->format('H:i'));
		} else {
			/* translators: Display format for task creation timestamp. 1: Date (today or specific), 2: Time */
			$return = ($today->format('Y-m-d') == $created_on->format('Y-m-d')) ? sprintf(esc_html('Created at %s today', 'zephyr-project-manager'), $created_on->format('H:i')) : sprintf(esc_html('Created on %1$s at %2$s', 'zephyr-project-manager'), $created_on->format('d M'), $created_on->format('H:i'));
		}
	...
	public static function new_project_cell($project, $args = []) {
		...
		if ($due_date->format('Y') == "-0001") {
			...
			$due_date = zpm_date($project->date_due);
		}
        ...
        <div class="zpm_project_grid_cell">
            <div class="zpm_project_grid_row zpm_project_item <?php esc_attr($project->type); ?>"
                 data-project-id="<?php echo esc_attr($project->id); ?>">
                <a href="<?php echo esc_url($url); ?>" data-project_id="<?php echo esc_attr($project->id); ?>"
                   class="zpm_project_title project_name" data-ripple="rgba(0,0,0,0.2)">
						...
						<div class="zpm_dropdown_menu" aria-hidden="true">
							<ul class="zpm_dropdown_list">
								<?php if (Utillities::canDeleteProject($userId, $project)) : ?>
                                    <li id="zpm_delete_project"><?php esc_html_e('Delete Project', 'zephyr-project-manager'); ?></li>
									<?php if ($project->archived) : ?>
                                        <li id="zpm-project-action__archive"
                                            data-archived="0"><?php esc_html_e('Unarchive Project', 'zephyr-project-manager'); ?></li>
									<?php else : ?>
                                        <li id="zpm-project-action__archive"
                                            data-archived="1"><?php esc_html_e('Archive Project', 'zephyr-project-manager'); ?></li>
									<?php endif; ?>
								<?php endif; ?>
								...
								<li id="zpm_export_project"
									...
									<div class="zpm_export_dropdown zpm_submenu_item">
										<ul>
											<li id="zpm_export_project_to_csv"
                                                class="zpm_project_option_sub"><?php esc_html_e('Export to CSV', 'zephyr-project-manager'); ?></li>
											<li id="zpm_export_project_to_json"
                                                class="zpm_project_option_sub"><?php esc_html_e('Export to JSON', 'zephyr-project-manager'); ?></li>
										</ul>
									</div>
								</li>
								<?php if (!$args['is_dashboard_project']) : ?>
                                    <li id="zpm_add_project_to_dashboard"><?php esc_html_e('Add to Dashboard', 'zephyr-project-manager'); ?></li>
								<?php else : ?>
                                    <li id="zpm-remove-from-dashboard"><?php esc_html_e('Remove from Dashboard', 'zephyr-project-manager'); ?></li>
								<?php endif; ?>
							</ul>
						</div>
					</span>
                ...
                <div class="zpm_project_body">
					<?php if (Projects::hasStatus($project)): ?>
                        <div class="zpm-project-status">
                            <span class="zpm-project-status-label"
                                  style="background: <?php echo esc_attr($status['color']); ?>"><?php echo esc_html($status['name']); ?></span>
                        </div>
					<?php endif; ?>

                    <span class="zpm_project_description project_description"><?php echo wp_kses_post($project->description); ?></span>
                    <div id="zpm_project_progress" data-zpm-link="<?php echo $url . '#tasks'; ?>">
						<span class="zpm_project_stat">
							<p class="zpm_stat_number"><?php echo esc_html($completed_tasks); ?></p>
							<p><?php esc_html_e('Completed Tasks', 'zephyr-project-manager'); ?></p>
						</span>
                        <span class="zpm_project_stat">
							<p class="zpm_stat_number"><?php echo esc_html($active_tasks); ?></p>
							<p><?php esc_html_e('Active Tasks', 'zephyr-project-manager'); ?></p>
						</span>
                        <span class="zpm_project_stat">
							<p class="zpm_stat_number"><?php echo esc_html($overdueTasks); ?></p>
							<p><?php esc_html_e('Overdue Tasks', 'zephyr-project-manager'); ?></p>
						</span>
                    ...
                    <div class="zpm_project_progress_bar_background">
                        <div class="zpm_project_progress_bar" data-total_tasks="<?php echo esc_attr($total_tasks); ?>"
                             data-completed_tasks="<?php echo esc_attr($completed_tasks); ?>"></div>
                    </div>
								...
								foreach ((array) $team as $member):
									...
									if (!Members::isTeam($member)) {
                                        ...
                                        <span class="zpm_avatar_container">
											...
											<span class="zpm_avatar_image"
                                                  title="<?php echo esc_attr($member['name']); ?>"
                                                  style="background-image: url(<?php echo esc_url($member['avatar']); ?>);">
											</span>
										</span>

										<?php
									} else {
										$team = Members::get_team($member);
                                        ...
                                        <span class="zpm_avatar_container">
											...
											<span class="zpm_avatar_image"
                                                  ...
                                                  style="background-image: url('https://ui-avatars.com/api/?background=<?php echo $color; ?>&color=fff&name=<?php echo $team['name']; ?>');">
											</span>
										</span>
		...
		if (is_array($project_members)) {
			foreach ($project_members as $member) {
				if (strpos($member, 'team') !== false) {
					$teamID = str_replace('team_', '', $member);
					$team = Members::get_team($teamID);
					if (Members::isTeamMember($team, $user_id)) {
						return true;
					}
				}
			}
	...
	public static function isTeamMember($project, $userID) {
		...
		if (in_array((int) $userID, (array) $projectMembers)) {
			return true;
		}
	...
	public static function frontend_project_item($project, $theme = 'default') {
        ...
        <li class="zpm-project-item col-md-12" data-project-id="<?php echo esc_attr($project->id); ?>">
            ...
            <a class="zpm-project-item-title"
               href="?action=project&id=<?php echo esc_attr($project->id); ?>"><?php echo stripslashes(esc_html($project->name)); ?>
				<?php if ($general_settings['display_project_id'] == '1') : ?>
            ...
            <span class="zpm-project-list-details"><?php echo esc_html($date_created); ?>
				<span class="zpm-project-item__due_date">
					<?php esc_html_e('Due Date', 'zephyr-project-manager'); ?>: <?php echo esc_html($due_date); ?>
				</span>
			</span>
            ...
            <p class="zpm-error-subtle"><?php esc_html_e('No description', 'zephyr-project-manager'); ?></p>
		<?php endif; ?>
            </p>
            ...
            <div class="zpm-project-item__footer">
				<?php if (is_array($categories)) : ?>
					<?php foreach ($categories as $category) : ?>
						...
						<?php if (!is_null($category)) : ?>
                            <span class="zpm-project-footer__category"
                                  style="background-color: <?php echo esc_attr($category->color); ?>"><?php echo esc_html($category->name); ?></span>
						<?php endif; ?>
					<?php endforeach; ?>
				<?php endif; ?>
            </div>
	...
	public static function get_attachments($project_id = null) {
		...
		$attachments = $wpdb->get_results($wpdb->prepare($query, $prepare));
		$attachments_array = [];
	...
	public static function file_html($attachment_id, $comment_id) {
		...
		$isImage = (in_array(strtolower($attachmentType), [
				'png',
				'jpg',
				'jpeg',
				'gif',
        ...
        <div class="zpm_file_item_container" data-project-id="<?php echo esc_attr($project_id); ?>">
            <div class="zpm_file_item" data-attachment-id="<?php echo esc_attr($attachment_id); ?>"
                 data-attachment-url="<?php echo esc_attr($attachment_url); ?>"
                 data-attachment-name="<?php echo esc_attr($attachment_name); ?>" data-task-name="None"
                 data-attachment-date="<?php echo esc_attr($attachment_date); ?>">
				<?php if ($isImage) : ?>
                    <!-- If attachment is an image -->
                    <div class="zpm_file_preview" data-zpm-action="show_info">
                        <span class="zpm_file_image"
                              style="background-image: url(<?php echo esc_url($attachment_url); ?>);"></span>
                    </div>
				<?php else : ?>
                    <div class="zpm_file_preview" data-zpm-action="show_info">
	...
	public static function project_modal() {
        ...
        <div id="zpm_project_modal" class="zpm-modal" role="form"
             aria-label="<?php esc_attr_e('New Project', 'zephyr-project-manager') ?>" aria-modal="true"
             aria-hidden="true">
            <div class="zpm_modal_body">
                <h3><?php esc_html_e('Create a new project', 'zephyr-project-manager'); ?></h3><span
                        class="zpm_close_modal lnr lnr-cross"></span>

                <div class="zpm-new-project__field">
                    <label class="zpm_label"
                           for="zpm-new-project--name"><?php esc_html_e('Project Name', 'zephyr-project-manager'); ?></label>
                    <input class="zpm_project_name_input zpm_input" id="zpm-new-project--name" name="zpm_project_name"
                           placeholder="<?php esc_html_e('Add a project name', 'zephyr-project-manager'); ?>"/>
                </div>

                <div class="zpm-new-project__field">
                    <label class="zpm_label"
                           for="zpm-new-project--description"><?php esc_html_e('Project Description', 'zephyr-project-manager'); ?></label>
                    <textarea id="zpm-new-project-description" id="zpm-new-project--description" class="zpm_input"
                              placeholder="<?php esc_html_e('Project Description', 'zephyr-project-manager'); ?>"></textarea>
                </div>

                <div class="zpm-new-project__field">
                    <label class="zpm_label"
                           for="zpm-new-project__managers"><?php esc_html_e('Project Managers', 'zephyr-project-manager'); ?></label>
                    <select id="zpm-new-project__managers" class="zpm_input zpm-input-chosen" multiple
                            data-placeholder="<?php esc_html_e('Select Project Managers', 'zephyr-project-manager'); ?>">
                ...
                <div class="zpm-new-project__field">
                    <label class="zpm_label"
                           for="zpm-new-task__status"><?php esc_html_e('Categories', 'zephyr-project-manager'); ?></label>
                    <select id="zpm-new-project__categories" class="zpm_input zpm-input-chosen" multiple
                            data-placeholder="<?php esc_html_e('Select Categories', 'zephyr-project-manager'); ?>">
						<?php foreach ($categories as $category) : ?>
                        ...
                        <div class="zpm_modal_item">
                            <div class="image zpm_project_selected" data-project-type="list">
                                <img class="zpm_selected_image"
                                     src="<?php echo esc_url(ZPM_PLUGIN_URL . '/assets/img/project_list_selected.png'); ?>"
                                     alt="<?php esc_attr_e('Click to change to the list view', 'zephyr-project-manager'); ?>"/>
                                <img src="<?php echo esc_url(ZPM_PLUGIN_URL . "/assets/img/project_list.png"); ?>"/>
                            </div>
                            <h4 class="title"><?php esc_html_e('List', 'zephyr-project-manager'); ?></h4>
                            <p class="description"><?php esc_html_e('Organize your work in an itemized list.', 'zephyr-project-manager'); ?></p>
                        </div>
                ...
                <div class="zpm_modal_buttons">
                    ...
                    <button id="zpm-new-project-priority" class="zpm_button zpm-priority-selection"
                            zpm-toggle-dropdown="zpm-new-project-priority-dropdown" data-priority="priority_none"><span
                                class="zpm-priority-name"><?php esc_html_e('Priority', 'zephyr-project-manager'); ?>: <?php esc_html_e('None', 'zephyr-project-manager'); ?></span>
                        <div id="zpm-new-project-priority-dropdown" class="zpm-dropdown zpm-priority-dropdown">
                            ...
                            <div class="zpm-dropdown-item zpm-new-project-priority" data-value="priority_none"
                                 data-color="#f9f9f9"><span
                                        class="zpm-priority-indicator zpm-color-none"></span><?php esc_html_e('None', 'zephyr-project-manager'); ?>
                            </div>
							...
							<?php foreach ($statuses as $slug => $status) : ?>
                                <div class="zpm-dropdown-item zpm-new-project-priority"
                                    ...
                                    <span class="zpm-priority-indicator <?php echo esc_attr($slug); ?>"
                                          style="background-color: <?php echo esc_html($status['color']); ?>"></span>
                    ...
                    <button id="zpm_modal_add_project"
                            class="zpm_button"><?php esc_html_e('Create Project', 'zephyr-project-manager'); ?></button>

					<?php if (!BaseController::is_pro()) : ?>
                        <p class="zpm-pro-upselling"><?php esc_html_e('Create Kanban-style board projects with the ', 'zephyr-project-manager'); ?>
                            <a class="zpm-pro-link" href="https://zephyr-one.com/purchase-pro"
                               target="_blank"><?php esc_html_e('Pro Version', 'zephyr-project-manager'); ?></a>.</p>
					<?php endif; ?>
                </div>
            </div>
        </div>
	...
	public static function copy_project($args = null) {
		...
		$messagesTable = ZPM_MESSAGES_TABLE;
		$i = $j = 0;
		$comments = Projects::get_comments($args['project_id']);
		foreach ($comments as $comment) {
			$commentData = $comment;
			$commentData->subject_id = $last_id;
			$commentData = (array) $commentData;
			unset($commentData['id']);
			$wpdb->insert($messagesTable, (array) $commentData);
		}
		if ((in_array('tasks', $args['copy_options']))) {
			foreach ($tasks as $task) {
				...
				$comments = Tasks::get_comments($task->id);
				foreach ($comments as $comment) {
					$commentData = $comment;
					$commentData->subject_id = $last_task_id;
					$commentData = (array) $commentData;
					unset($commentData['id']);
					$wpdb->insert($messagesTable, (array) $commentData);
				}
				...
				foreach ($subtasks as $subtask) {
					...
					$lastSubtaskID = $wpdb->insert_id;
					$comments = Tasks::get_comments($subtask->id);
					foreach ($comments as $comment) {
						$commentData = $comment;
						$commentData->subject_id = $lastSubtaskID;
						$commentData = (array) $commentData;
						unset($commentData['id']);
						$wpdb->insert($messagesTable, (array) $commentData);
					}
					foreach ($subtasks as $subtask) {
						...
						foreach ($comments as $comment) {
							...
							$commentData = (array) $commentData;
							unset($commentData['id']);
							$wpdb->insert($messagesTable, $commentData);
						}
					}
				}
			}
	...
	public static function add_to_dashboard($project_id) {
		$option = maybe_unserialize(get_user_meta(get_current_user_id(), 'zpm_dashboard_projects', true));
		$option = (array) $option;
		if (!in_array($project_id, $option)) {
			$option[] = $project_id;
		}
		update_user_meta(get_current_user_id(), 'zpm_dashboard_projects', serialize($option));
	}
	...
	public static function get_dashboard_projects($object = true, $userID = null) {
		if (is_null($userID)) {
			$userID = get_current_user_id();
		}
		$results = [];
		$ids = maybe_unserialize(get_user_meta($userID, 'zpm_dashboard_projects', true));
		$ids = (array) $ids;
		$projects = apply_filters('zpm_filter_global_projects', Projects::get_projects(null, null, null, false, $userID));
		if (!$object) {
			$results = $ids;
		} else {
			foreach ($ids as $id) {
	...
	public static function remove_from_dashboard($project_id) {
		...
		update_user_meta(get_current_user_id(), 'zpm_dashboard_projects', serialize($dashboard_projects));
	}
	...
	public static function search($query) {
		...
		$result_projects = [];
		$added = [];
		$projectSearchQuery = apply_filters('zpm/search/projects/query', "SELECT id, name FROM `{$table_name}` WHERE name LIKE %s OR id LIKE %s", $query);
		$projectSearchArgs = apply_filters('zpm/search/projects/args', [
			'%' . $wpdb->esc_like($query) . '%',
			'%' . $wpdb->esc_like($query) . '%'
		], $query);
		$results = $wpdb->get_results($wpdb->prepare($projectSearchQuery, $projectSearchArgs));
	...
	public static function view_project_container($project_id = null) {
		?>
        <div id="zpm_quickview_modal" class="zpm-modal"
             data-project-id="<?php echo !is_null($project_id) ? esc_attr($project_id) : ''; ?>" aria-modal="true"
	...
	public static function get_task_count($id) {
		...
		$count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table_name WHERE project = %d AND parent_id = '-1'", $id));
	...
	public static function get_completed_task_count($id) {
		global $wpdb;
		$table_name = ZPM_TASKS_TABLE;
		$count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table_name WHERE project = %d AND completed = '1' AND parent_id = '-1'", $id));
	...
	public static function get_active_task_count($id) {
		global $wpdb;
		$table_name = ZPM_TASKS_TABLE;
		$count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table_name WHERE project = %s AND completed = '0'", $id));
		...
		if (($handle = fopen($file, "r")) !== false) {
			...
			while (($data = fgetcsv($handle, 1000, ",")) !== false) {
				if ($row > 0) {
					$num = count($data);
					$project = array(
						'id' => zpm_sanitize_int($data[0]),
						'user_id' => zpm_sanitize_int($data[1]),
						'name' => sanitize_text_field($data[2]),
						'description' => sanitize_text_field($data[3]),
						'completed' => sanitize_text_field($data[4]),
						'team' => sanitize_text_field($data[5]),
						'categories' => sanitize_text_field($data[6]),
						'date_created' => sanitize_text_field($data[7]),
						'date_due' => sanitize_text_field($data[8]),
						'date_start' => sanitize_text_field($data[9]),
						'date_completed' => sanitize_text_field($data[10]),
						'other_data' => sanitize_text_field($data[11])
					);
	...
	public static function loadFromJSON($file) {
		...
		foreach ($jsonArray as $project) {
			$project = array(
				'id' => zpm_sanitize_int($project->id),
				'parent_id' => zpm_sanitize_int($project->parent_id),
				'user_id' => zpm_sanitize_int($project->user_id),
				'name' => sanitize_text_field($project->name),
				'description' => sanitize_text_field($project->description),
				'categories' => sanitize_text_field($project->categories),
				'completed' => sanitize_text_field($project->completed),
				'date_created' => sanitize_text_field($project->date_created),
				'date_start' => sanitize_text_field($project->date_start),
				'date_due' => sanitize_text_field($project->date_due),
				'date_completed' => sanitize_text_field($project->date_completed),
				'team' => sanitize_text_field($project->team),
				'custom_fields' => sanitize_text_field($project->custom_fields)
			);
			$projects[] = $project;
		}
	...
	public static function hasStatus($project) {
		...
		if ($status == 'priority_none' || empty($status) || $status == 'none') {
			return false;
		}
	...
	public static function getEstimatedCompletionDate($projectID) {
		...
		if (is_null($latestDate) || empty($latestDate) || !zpm_is_date_valid($latestDate)) {
			return __('None', 'zephyr-project-manager');
		}
	...
	public static function shortcodeHtml($project, $a = []) {
		if (!is_object($project)) {
			return;
		}
		$a = shortcode_atts([
        ...
        <div class="zpm-project-shortcode" data-project-id="<?php echo esc_attr($project->id); ?>"
             data-edit-project="<?php echo $a['edit'] ? 'true' : 'false'; ?>">
            <div class="zpm-project-card <?php echo esc_attr($classes); ?>">
                ...
                <div class="zpm-project-card-header">
                    <small class="zpm-project-card-type"><?php esc_html_e('Project', 'zephyr-project-manager'); ?></small>
                ...
                <div class="zpm-project-card-body">
                    ...
                    <div id="zpm-project-shortcode-progress">
                        ...
                        <span class="zpm-project-shortcode-stat">
							...
							<p><?php esc_html_e('Messages', 'zephyr-project-manager'); ?></p>
						</span>
                    </div>

                    <div class="zpm-project-shortcode__tasks">
						<?php foreach ((array) $tasks as $task) : ?>
							<?php echo Projects::shortcodeTaskHtml($task, $project); ?>
						<?php endforeach; ?>
                    </div>
					...
					<?php if (Projects::hasStatus($project)) : ?>
                        <div class="zpm-project-status">
                            <span class="zpm-project-status-label"
                                  style="background: <?php echo !empty($status['color']) ? esc_attr($status['color']) : 'var(--zpm-primary-color)'; ?>; color: <?php echo esc_attr(Utillities::getTextColorBasedOnBackgroundColor($status['color'])); ?>;"><?php echo esc_html($status['name']); ?></span>
                        </div>
					<?php endif; ?>

                    <div class="zpm-shortcode-progress-background">
                        <div class="zpm-shortcode-progress-bar" data-total_tasks="<?php echo esc_attr($total_tasks); ?>"
                             data-completed_tasks="<?php echo esc_attr($completed_tasks); ?>"></div>
                    </div>
                </div>
	...
	public static function shortcodeTaskHtml($task, $project) {
		ob_start();
		?>
        <div class="zpm-project-shortcode__task <?php echo $task->completed == '1' ? 'zpm-task-completed' : ''; ?>"
			...
			<?php endif; ?>
			<?php $subtasks = Tasks::get_subtasks($task->id); ?>
			<?php if (!empty($subtasks)): ?>
                <div class="zpm-project-shortcode__task-subtasks">
					<?php foreach ($subtasks as $subtask): ?>
						<?php echo Projects::shortcodeTaskHtml($subtask, $project); ?>
					<?php endforeach; ?>
                </div>
			<?php endif; ?>
	...
	public static function isValid($project) {
		if (is_object($project)) {
			return true;
		}
	...
	public static function filterCompleted($projects) {
		$results = [];
		foreach ($projects as $project) {
			if (Projects::isCompleted($project)) {
				$results[] = $project;
