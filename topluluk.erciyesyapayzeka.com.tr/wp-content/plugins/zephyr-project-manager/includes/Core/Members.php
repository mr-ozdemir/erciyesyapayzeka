...
class Members {
	 ...
	 */
	public static function get_zephyr_members(): array {
		$manager = ZephyrProjectManager();
		$users = $manager::get_users(false);
		$members = array();
		$currentUserID = get_current_user_id();
...
?>
		<div class="zpm_team_member" data-team-id="<?php echo esc_attr($team['id']); ?>">
			<div class="zpm_member_details" data-ripple="rgba(0,0,0,0.1)" zpm-ripple>
				...
				<h3 class="zpm-team-members-title"><?php esc_html_e('Members', 'zephyr-project-manager'); ?></h3>
				...
				<?php if ($member_count <= 0) : ?>
					<p class="zpm-team-no-members"><?php esc_html_e('No members have been added to this team', 'zephyr-project-manager'); ?></p>
				<?php endif; ?>

				<div class="zpm-team-options-btns">
					<button class="zpm_button zpm-delete-team" data-team-id="<?php echo esc_attr($team['id']); ?>"><?php esc_html_e('Delete', 'zephyr-project-manager') ?></button>
					<button class="zpm_button zpm-edit-team" data-team-id="<?php echo esc_attr($team['id']); ?>" data-zpm-modal-trigger="zpm-edit-team-modal"><?php esc_html_e('Edit Team', 'zephyr-project-manager'); ?></button>
				</div>
			</div>
		</div>
	<?php
	...
	?>
		<select id="<?php echo $id !== '' ? esc_attr($id) : 'zpm-team-select-dropdown'; ?>" class="zpm-team-select-dropdown zpm_input zpm-input-chosen">
			<option value="-1"><?php esc_html_e('Select Team', 'zephyr-project-manager'); ?></option>
			...
			<?php if (Members::canEditMemberAccess() && intval($member['id']) !== intval($userID)) : ?>
				<span class="zpm-user-row__access-controls">
					<label for="zpm-can-zephyr-<?php echo esc_attr($member['id']); ?>" class="zpm_checkbox_label">
						...
						<?php if (!Utillities::canZephyr($member['id'])): ?>
							<span> (<?php esc_html_e('Role Not Allowed', 'zephyr-project-manager'); ?>)</span>
						<?php endif; ?>
					</label>
				</span>
			<?php endif; ?>
	...
	public static function tableItemHtml($member) {
		$edit_url = admin_url('/admin.php?page=zephyr_project_manager_teams_members') . '&action=edit_member&user_id=' . $member['id'];
		$userID = get_current_user_id();
		ob_start();
...
<?php
	...
	public static function isTeam($id) {
	...
	public static function canViewMember($memberID) {
		...
		if (zpm_sanitize_bool($generalSettings['view_members']) == true) return true;
