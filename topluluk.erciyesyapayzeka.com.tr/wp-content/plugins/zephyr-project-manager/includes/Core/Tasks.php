...
class Tasks {
	...
	public function filter_task_data($task) {
		...
		$start_date = zpm_is_date_valid($task->date_start) ? date_i18n($this->settings['date_format'], strtotime($task->date_start)) : __('Not set', 'zephyr-project-manager');
		$due_date = zpm_is_date_valid($task->date_start) ? date_i18n($this->settings['date_format'], strtotime($task->date_due)) : __('Not set', 'zephyr-project-manager');
	...
	public static function get_tasks($args = null) {
		...
		$tasks = Tasks::removeDuplicates($tasks);
	...
	public static function getAllTasks($args = null) {
		...
		$tasks = $wpdb->get_results($wpdb->prepare($query, $prepare));
		$tasks = Tasks::sortByStartDate($tasks);

		return $tasks;
	}

	public static function removeDuplicates($tasks) {
		$uniqueIds = [];
		foreach ($tasks as $index => $item) {
			if (in_array($item->id, $uniqueIds)) {
				// Duplicate found, remove item
				unset($tasks[$index]);
			} else {
				// Add the 'id' to the list of unique 'id's
				$uniqueIds[] = $item;
			}
		}
	...
	public static function get_task($task_id) {
		...
		$task = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE id = %d", intval($task_id)));
	...
	public static function copy($id, $extra_args = null) {
		...
		foreach ($subtasks as $subtask) {
			$subtaskArgs = (array) $subtask;
			$subtaskArgs['parent_id'] = $newTaskID;
			unset($subtaskArgs['id']);
			// $subtaskArgs['assignee'] = $subtask->assignee;
			// $subtaskArgs['user_id'] = $subtask->user_id;
			// $subtaskArgs['name'] = $subtask->name;
			// $subtaskArgs['completed'] = $subtask->completed;
			// $subtaskArgs['date_start'] = $subtask->date_start;
			// $subtaskArgs['date_due'] = $subtask->date_due;
			// $subtaskArgs['date_created'] = $subtask->date_created;
			// $subtaskArgs['description'] = $subtask->description;
			Tasks::copy($subtask->id, $subtaskArgs);
		}
	...
	public static function task_exists($task_id) {
		global $wpdb;
		$table_name = ZPM_TASKS_TABLE;
		$tasks = $wpdb->query($wpdb->prepare("SELECT * FROM $table_name WHERE id = %d", intval($task_id)));
	...
	public static function get_completed_task_count() {
		global $wpdb;
		$table_name = ZPM_TASKS_TABLE;
		$task_count = $wpdb->query($wpdb->prepare("SELECT id FROM $table_name WHERE completed = %d AND parent_id = %d", 1, - 1));
	...
	public static function sortByDueDate($tasks, $asc = false) {
		$sorted_array = [];
		foreach ($tasks as $task) {
			//$key = date('Y-m-d',strtotime($task->date_start));
			$random = $task->id; // Utillities::generate_random_string(6);
			$key = $task->date_due . $task->id;
		...
		return $asc ? array_reverse($tasks) : $tasks;
	}
	...
	public static function get_project_tasks($project_id, $showSubtasks = null) {
		global $wpdb;
		$table_name = ZPM_TASKS_TABLE;
		$query = "SELECT * FROM $table_name WHERE project = %d AND parent_id = '-1' AND archived = '0'";
		// if ($subtasks == null) {
	...
	public static function get_project_task_count($project_id) {
		global $wpdb;
		$table_name = ZPM_TASKS_TABLE;
		$tasks = $wpdb->query($wpdb->prepare("SELECT id FROM $table_name WHERE project = %d AND parent_id = %d", $project_id, - 1));
	...
	public static function get_project_completed_tasks($project_id) {
		global $wpdb;
		$table_name = ZPM_TASKS_TABLE;
		$tasks = $wpdb->query($wpdb->prepare("SELECT id FROM $table_name WHERE project = %d AND completed = %d AND parent_id = %d", $project_id, 1, - 1));
	...
	public static function task_created_by($task_id) {
		global $wpdb;
		$table_name = ZPM_TASKS_TABLE;
		$query = $wpdb->prepare("SELECT user_id, date_created FROM {$table_name} WHERE id = %d", $task_id);
		...
		/* translators: Display format for creation timestamp. 1: User name, 2: Date (today or specific), 3: Time */
		$return = ($today->format('Y-m-d') == $fullDate) ? sprintf(esc_html('Created by %1$s at %2$s today', 'zephyr-project-manager'), $username, $time) : sprintf(esc_html('Created by %1$s on %2$s at %3$s', 'zephyr-project-manager'), $username, $date, $time);
	...
	public static function get_attachments() {
		global $wpdb;
		$table_name = ZPM_MESSAGES_TABLE;
		$attachments = $wpdb->get_results($wpdb->prepare("SELECT id, parent_id, user_id, subject, subject_id, message, type, date_created FROM {$table_name} WHERE subject = %s", 'task'));
		$attachments_array = [];
	...
	public static function view_container($task_id = null) {
		?>
        <div id="zpm_task_view_container" class="zpm-modal" data-task-id="<?php echo esc_attr($task_id); ?>"
	...
	public static function new_task_row($task, $frontend = false, $filters = []) {
		...
		if (isset($filters['assignee'])) {
			...
			if (!in_array($assigneeFilter, ['-1', 'all'])) {
				if (!Tasks::is_assignee($task, $assigneeFilter)) {
					$canView = false;
				}
			} else {
		...
		if ($canView) {
			...
			if (Tasks::hasProject($task)) {
				...
				$projectURL = Utillities::getURL('project', [
					'id' => $task_project->id,
				], '', $frontend);
			}
			...
			$row_classes = apply_filters('zpm/task/classes', $row_classes, $task);
			ob_start(); ?>
            <a href="<?php echo esc_url($task_url); ?>" class="zpm_task_list_row <?php echo esc_attr($row_classes); ?>"
               data-project-url="<?php echo $projectURL; ?>" data-task-id="<?php echo esc_attr($task->id); ?>"
               ripple="ripple" data-ripple="rgba(0,0,0,0.1)" data-task-name="<?php echo esc_html($task->name); ?>"
               id="zpm-task-list__item-<?php echo esc_attr($task->id); ?>" data-due="<?php echo $task->date_due; ?>">
				<?php if (apply_filters('zpm_can_complete_task', true, $task)) : ?>
                    <label for="zpm_task_id_<?php echo esc_attr($task->id); ?>" class="zpm-material-checkbox">
                        <input type="checkbox" id="zpm_task_id_<?php echo esc_attr($task->id); ?>"
                               name="zpm_task_id_<?php echo esc_attr($task->id); ?>"
                               class="zpm_task_mark_complete zpm_toggle invisible"
                               value="1" <?php echo $task->completed == '1' ? 'checked' : ''; ?>
                               data-task-id="<?php echo esc_attr($task->id); ?>"
                               aria-label="<?php esc_attr_e('Toggle task completion', 'zephyr-project-manager'); ?>">
                ...
                <span class="zpm_task_details">
					<?php if ($project_name !== '' && !zpm_is_single_project()) : ?>
                        <span data-zpm-link="<?php echo $projectURL; ?>"
                              title="<?php esc_html_e('Project', 'zephyr-project-manager'); ?>"
                              class="zpm_task_project"><?php echo esc_html($project_name); ?></span>
					<?php endif; ?>
					...
					<?php if (property_exists($task, 'team') && $task->team !== "") : ?>
						...
						<?php if (isset($team['name']) && $team['name'] !== "" && !empty($team['name'])) : ?>
                            <span title="Team"
                                  class="zpm_task_project zpm-task-team"><?php echo esc_html($team['name']); ?></span>
						<?php endif; ?>
					<?php endif; ?>
					...
					<?php if (!empty($assignees)) : ?>
                        <span class="zpm-task_assignees">
							<?php foreach ($assignees as $assignee) : ?>
                                <span title="<?php echo esc_attr($assignee['name']); ?>" class='zpm_task_assignee'
                                      style='background-image: url("<?php echo esc_url($assignee['avatar']) ?>"); <?php echo $assignee['avatar'] == '' ? 'display: none;' : ''; ?>'
                                      title="<?php echo esc_attr($assignee['name']) ?>"></span>
							<?php endforeach; ?>
						</span>
					<?php endif; ?>
					...
					<span class="zpm_task_due_date <?php echo $overdue ? 'zpm_overdue' : ''; ?>">
						...
						<?php endif; ?>
						<?php if ($type == 'daily'): ?>
                            <span class="zpm-task__type-label"><?php esc_html_e('Daily', 'zephyr-project-manager'); ?></span>
						<?php endif; ?>
						<?php if ($type == 'weekly'): ?>
                            <span class="zpm-task__type-label"><?php esc_html_e('Weekly', 'zephyr-project-manager'); ?></span>
						<?php endif; ?>
						<?php if ($type == 'monthly'): ?>
                            <span class="zpm-task__type-label"><?php esc_html_e('Monthly', 'zephyr-project-manager'); ?></span>
						<?php endif; ?>
						<?php if ($type == 'annually'): ?>
                            <span class="zpm-task__type-label"><?php esc_html_e('Annualy', 'zephyr-project-manager'); ?></span>
						<?php endif; ?>
					</span>
					<?php do_action('zpm_task_row_details', $task); ?>
					<label for="zpm-task-bulk-selector-<?php echo esc_attr($task->id); ?>"
                           class="zpm-material-checkbox zpm-task-bulk-selector-wrap">
						<input type="checkbox" id="zpm-task-bulk-selector-<?php echo esc_attr($task->id); ?>"
                               name="zpm-task-bulk-selector-<?php echo esc_attr($task->id); ?>"
                               class="zpm-task-bulk-selector zpm_toggle invisible" value="1"
                               data-task-bulk-selector="<?php echo esc_attr($task->id); ?>">
						<span class="zpm-material-checkbox-label"></span>
					</label>
				</span>
                ...
                <div class="zpm-task-actions__hidden">
					<span class="zpm-task__delete-checkbox">
						<label for="zpm-delete-task__checkbox<?php echo esc_attr($task->id); ?>"
                               class="zpm-material-checkbox">
							<input type="checkbox" id="zpm-delete-task__checkbox<?php echo esc_attr($task->id); ?>"
                                   name="zpm-delete-task__checkbox<?php echo esc_attr($task->id); ?>"
                                   class="zpm-delete-task__checkbox zpm_toggle invisible" value="1"
                                   data-task-id="<?php echo esc_attr($task->id); ?>">
							<span class="zpm-material-checkbox-label"></span>
						</label>
					</span>
                ...
                <span class="zpm-task-priority-display <?php echo esc_attr($priority); ?>"
                      style="background: <?php echo $status['color'] !== '' ? esc_attr($status['color']) : ''; ?>"></span>
            </a>
			<?php
			$html = ob_get_clean();
			$html = apply_filters('zpm/task/html', $html, $task);
		}
		...
		foreach ($subtasks as $subtask) {
			if (!empty($filter)) {
				...
				if (isset($filters['status'])) {
					if (!in_array($filters['status'], ['all', '-1', 'active', 'archived'])) {
						...
						if (!Tasks::isStatus($subtask, $status)) {
							continue;
						}
					}
				}
			...
			$subtask->date_due = $task->date_due;
	...
	public static function search($query) {
		...
		$added = [];
		$result_tasks = [];
		$taskSearchQuery = apply_filters('zpm/search/tasks/query', "SELECT id, name FROM `{$table_name}` WHERE name LIKE %s OR description LIKE %s", $query);
		$taskSearchArgs = apply_filters('zpm/search/tasks/args', [
			'%' . $wpdb->esc_like($query) . '%',
			'%' . $wpdb->esc_like($query) . '%'
		], $query);
		$results = $wpdb->get_results($wpdb->prepare($taskSearchQuery, $taskSearchArgs));
		foreach ($results as $result) {
			$result_tasks[] = $result;
			$added[] = $result->id;
		}
	...
	public static function template_row_html($id) {
        ...
        <div class="zpm-custom-task-template" data-template-id="<?php echo esc_attr($template['id']); ?>">
            ...
            <label for="zpm-task-template-checkbox-<?php echo esc_attr($template['id']); ?>"
                   class="zpm-material-checkbox">
                <input type="checkbox" id="zpm-task-template-checkbox-<?php echo esc_attr($template['id']); ?>"
                       name="zpm_can_zephyr" class="zpm-default-task-template zpm_toggle invisible" value="1"
                       data-template-id="<?php echo esc_attr($template['id']); ?>" <?php echo $is_default ? 'checked' : ''; ?>>
                <span></span>
            </label>
            ...
            <span class="zpm-remove-task-template lnr lnr-cross"
                  data-template-id="<?php echo esc_html($template['id']); ?>"></span>
        </div>
		<?php
		$html = ob_get_clean();
	...
	public static function get_assignees($task, $get_object = false) {
		...
		if (!property_exists($task, 'assignee')) {
			return [];
		}
		$added = [];
	...
	public static function getArchivedTasks() {
		...
		$results = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$tableName} WHERE archived = %d", 1));
	...
	public static function subtaskItemHtml($task, $classes = '', $level = 0, $visited = []) {
		...
		ob_start();
		?>
        <div class="zpm_subtask_item <?php echo $task->isCompleted() ? 'zpm_task_complete' : ''; ?> <?php echo $hasChildren ? 'has-children' : ''; ?> <?php echo $level > 0 ? "zpm-sub-subtask zpm-subtask-item--sub zpm-subtask-level-{$level}" : ''; ?> <?php echo esc_attr($classes); ?>"
             data-zpm-subtask="<?php echo esc_attr($task->id); ?>">
            <div class="zpm-subtask-item--content">
                <label for="zpm_subtask_<?php echo esc_attr($task->id); ?>" class="zpm-material-checkbox">
                    <input type="checkbox" id="zpm_subtask_<?php echo esc_attr($task->id); ?>"
                           class="zpm_subtask_is_done"
                           data-task-id="<?php echo esc_attr($task->id); ?>" <?php echo $task->isCompleted() ? 'checked' : ''; ?>>
                    <span class="zpm-material-checkbox-label"></span>
                </label>
                <span class="zpm_subtask_name"><a class="zpm_link"
                                                  href="<?php echo esc_url(Tasks::task_url($task->id, $frontend)); ?>"
                                                  target="_BLANK"><?php echo stripslashes(esc_html($task->name)); ?></a></span>
                ...
                <div class="zpm-subtask-actions">
                    <span data-zpm-subtask-id="<?php echo esc_attr($task->id); ?>"
                          class="zpm_update_subtask"><?php esc_html_e('Save Changes', 'zephyr-project-manager'); ?></span>
					<?php if (Utillities::canDeleteTask(get_current_user_id(), $task)): ?>
                        <span data-zpm-subtask-id="<?php echo esc_attr($task->id); ?>"
                              class="zpm_delete_subtask"><?php esc_html_e('Delete', 'zephyr-project-manager'); ?></span>
					<?php endif; ?>
					<?php if ($level == 0): ?>
			...
			<?php if ($hasChildren) : ?>
                <div class="zpm-subtasks-container">
					<?php foreach ($subtasks as $subtask) : ?>
						<?php echo Tasks::subtaskItemHtml($subtask, $classes, $level + 1, $visited); ?>
					<?php endforeach; ?>
                </div>
			<?php endif; ?>
        </div>
	...
	public static function getProject($task, $visited = []) {
		$isSubtask = Tasks::isSubtask($task);
		$hasProject = Tasks::hasProject($task);
		if (isset($visited[$task->id]) && $visited[$task->id] == $task->parent_id) {
		...
		if (!$isSubtask) {
			...
			$visited[$task->id] = $task->parent_id;
			$rootTask = Tasks::getRoot($task, $visited);
			$hasProject = Tasks::hasProject($rootTask, $visited);
			if ($hasProject) {
        ...
        <div id="zpm_new_subtask_modal" class="zpm-modal zpm_compact_modal" aria-modal="true" aria-hidden="true">
            <div class="zpm-form__group">
                <input type="text" name="zpm_new_subtask_name" id="zpm_new_subtask_name" class="zpm-form__field"
                       placeholder="<?php esc_html_e('Subtask Name', 'zephyr-project-manager'); ?>"
                       data-ajax-name="name"/>
                <label for="zpm_new_subtask_name"
                       class="zpm-form__label"><?php esc_html_e('Subtask Name', 'zephyr-project-manager'); ?></label>
            </div>

            <div class="zpm-form__group">
                <textarea type="text" name="zpm-new-subtask__description" id="zpm-new-subtask__description"
                          class="zpm-form__field"
                          placeholder="<?php esc_html_e('Subtask Description', 'zephyr-project-manager'); ?>"
                          data-ajax-name="description"></textarea>
                <label for="zpm-new-subtask__description"
                       class="zpm-form__label"><?php esc_html_e('Subtask Description', 'zephyr-project-manager'); ?></label>
            </div>
                ...
                <div class="zpm-col zpm-col-6">
                    <div class="zpm-form__group">
                        <input type="text" name="zpm-new-subtask__start" id="zpm-new-subtask__start"
                               ...
                               placeholder="<?php esc_html_e('Start', 'zephyr-project-manager'); ?>"
                               data-ajax-name="start-date"/>
                        <label for="zpm-new-subtask__start"
                               class="zpm-form__label"><?php esc_html_e('Start', 'zephyr-project-manager'); ?></label>
                    </div>
                </div>
                <div class="zpm-col zpm-col-6">
                    <div class="zpm-form__group">
                        <input type="text" name="zpm-new-subtask__due" id="zpm-new-subtask__due"
                               class="zpm-form__field zpm-datepicker"
                               placeholder="<?php esc_html_e('Due', 'zephyr-project-manager'); ?>"
                               data-ajax-name="due-date"/>
                        <label for="zpm-new-subtask__due"
                               class="zpm-form__label"><?php esc_html_e('Due', 'zephyr-project-manager'); ?></label>
                    </div>
                </div>
            </div>

            <button id="zpm_save_new_subtask"
                    class="zpm_button"><?php esc_html_e('Create Subtask', 'zephyr-project-manager'); ?></button>
        </div>
		<?php
	}
	...
	public static function getTaskParents($task) {
		...
		while ($parentsDone !== 1) {
			if (!is_object($prevParent)) {
				...
				if (in_array($prevParent->id, $visited)) {
					$parentsDone = 1;
					break;
				} else {
					$visited[] = $prevParent->id;
	...
	public static function hasProject($task, $visited = []) {
		if (!is_object($task)) {
			return false;
		}
		if (isset($visited[$task->id]) && $visited[$task->id] == $task->parent_id) {
			return $task->project !== '-1' && empty($task->project);
		}
		$isSubtask = Tasks::isSubtask($task);
		if ($isSubtask) {
			$visited[$task->id] = $task->parent_id;
	...
	public static function contextMenu($taskID) {
		...
		ob_start(); ?>
        <span class="zpm_modal_options_btn zpm-task-context-menu" data-dropdown-id="zpm_view_task_dropdown"
              data-task-id="<?php echo esc_attr($taskID); ?>" aria-haspopup="true">
			<i class="dashicons dashicons-menu"></i>
			<div class="zpm_modal_dropdown" id="zpm_view_task_dropdown" aria-hidden="true">
				<ul class="zpm_modal_list">
					<?php if (Utillities::canDeleteTask(get_current_user_id(), $task)) : ?>
                        <li id="zpm_delete_task"><?php esc_html_e('Delete Task', 'zephyr-project-manager'); ?></li>
                        <li id="zpm_archive_task" data-archive-task="<?php echo esc_attr($taskID) ?>"
                            data-task-id="<?php echo esc_attr($taskID) ?>"
                            data-archived="<?php echo esc_attr($task->archived) ?>"><?php echo $task->archived == '1' ? __('Unarchive Task', 'zephyr-project-manager') : __('Archive Task', 'zephyr-project-manager'); ?></li>
					<?php endif; ?>
					<?php if (Utillities::can_create_tasks()) : ?>
                        <li id="zpm_copy_task"><?php esc_html_e('Copy Task', 'zephyr-project-manager'); ?></li>
					<?php endif; ?>
					<li id="zpm_export_task"><?php esc_html_e('Export Task', 'zephyr-project-manager'); ?>
						<div class="zpm_export_dropdown">
							<ul>
								<li id="zpm_export_task_to_csv"><?php esc_html_e('Export to CSV', 'zephyr-project-manager'); ?></li>
								<li id="zpm_export_task_to_json"><?php esc_html_e('Export to JSON', 'zephyr-project-manager'); ?></li>
							</ul>
						</div>
					</li>
					<?php if (Utillities::can_create_projects()) : ?>
                        <li id="zpm_convert_task"><?php esc_html_e('Convert to Project', 'zephyr-project-manager'); ?></li>
					<?php endif; ?>
					<li id="zpm_print_task"><?php esc_html_e('Print', 'zephyr-project-manager'); ?></li>
				</ul>
			</div>
		</span>
		<?php
	...
	public static function isOverdue($task) {
		...
		if (in_array($task->date_due, ['0000-00-00 00:00:00', '0000-00-00', ''])) {
			return false;
		}
		$dueTime = strtotime($task->date_due);
		$now = strtotime('now');
	...
	public static function syncIcs() {
		...
		foreach ($members as $member) {
			...
			$events = '';
			foreach ($tasks as $task) {
				if (!Tasks::is_assignee($task, $member['id'])) {
					continue;
				}
...
";
	...
	public static function taskListItemInput() {
        ...
        <div class="zpm-form__group zpm-new-task-field__name">
            <input type="text" name="zpm_new_task_name" id="zpm_new_task_name" class="zpm-form__field"
                   data-task-list-item placeholder="<?php esc_html_e('Enter Task Name', 'zephyr-project-manager'); ?>"
                   autocomplete="off">
            <label for="zpm_new_task_name"
                   class="zpm-form__label"><?php esc_html_e('Add Task Name', 'zephyr-project-manager'); ?></label>
        </div>
		<?php
		return ob_get_clean();
	}
	...
	public static function isSubtask($task) {
		...
		if (property_exists($task, 'parent_id')) {
			return intval($task->parent_id) !== - 1;
		} else if (property_exists($task, 'parentId')) {
			return intval($task->parentId) !== - 1;
		}

		return false;
	}

	public static function getRoot($task, $visited = []) {
		if (isset($visited[$task->id]) && $visited[$task->id] == $task->parent_id) {
			return $task;
		}
		if (Tasks::isSubtask($task)) {
			...
			return Tasks::getRoot(Tasks::get_task($task->parent_id), $visited);
		}
	...
	public static function getRecurringTasks($task) {
		$type = Tasks::get_type($task);
		if (in_array($type, ['default', 'normal'])) {
			return [];
		}
	...
	public static function sendComment($taskID, $message, $subject, $userID = '', $type = '', $parentID = '', $discussionId = 0) {
		global $wpdb;
		$tableName = ZPM_MESSAGES_TABLE;
		$timezone = zpm_get_timezone();
		$dt = new DateTime('now', $timezone);
		$date = $dt->format('Y-m-d H:i:s');
		$type = !empty($type) ? serialize($type) : '';
		$settings = array(
			'user_id' => $userID,
			'subject' => $subject,
			'subject_id' => $taskID,
