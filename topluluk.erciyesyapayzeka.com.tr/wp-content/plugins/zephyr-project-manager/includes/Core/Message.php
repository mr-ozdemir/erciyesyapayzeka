	...
	public function sendMentionEmails() {
		...
		if (preg_match_all($mentionRegex, $this->content, $matches)) {
			foreach ($matches[1] as $key => $match) {
				...
				/* translators: Mention notification. %s: Comment content */
			...
			<div data-zpm-comment-id="<?php echo esc_attr($this->id); ?>" class="zpm_comment <?php echo esc_attr($custom_classes); ?>">
				<div class="zpm-comment-bubble">
					...
					<span class="zpm_comment_user_text">
						...
						<div class="zpm_comment_content"><?php echo wp_kses_post($this->content); ?></div>

						<?php if (!empty($commentAttachments)) : ?>
							<ul class="zpm_comment_attachments"><p><?php esc_html_e( 'Attachments', 'zephyr-project-manager' ); ?>:</p>
								<?php foreach ($commentAttachments as $attachment) : ?>
									...
									<?php if ($isImage) : ?>
										<!-- Image -->
										<li class="zpm_comment_attachment" data-attachment="<?php echo esc_attr($attachment->id); ?>"><a class="zpm_link" href="<?php echo esc_url($attachmentUrl); ?>" download><img class="zpm-image-attachment-preview" src="<?php echo esc_url($attachmentUrl); ?>"></a></li>
									<?php else: ?>
										<?php if (!empty($attachmentPreviewIcon)): ?>
											...
											<div class="zpm-message-file-preview">
												...
												<li class="zpm_comment_attachment" data-attachment="<?php echo esc_attr($attachment->id); ?>"><a class="zpm_link" href="<?php echo esc_url($attachmentUrl); ?>" download><?php echo esc_html($attachmentName); ?></a></li>
											</div>
										<?php else: ?>
											<li class="zpm_comment_attachment" data-attachment="<?php echo esc_attr($attachment->id); ?>"><a class="zpm_link" href="<?php echo esc_url($attachmentUrl); ?>" download><?php echo esc_html($attachmentName); ?></a></li>
										<?php endif; ?>
