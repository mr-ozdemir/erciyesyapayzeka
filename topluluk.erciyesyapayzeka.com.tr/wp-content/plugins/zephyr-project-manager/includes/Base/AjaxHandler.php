...
class AjaxHandler extends BaseController {
	...
	public function updateStatusOrders() {
		$orders = $this->getPostVar('orders', []);
		update_option('zpm/status/orders', $orders);
		wp_send_json_success([]);
	}

	public function updatePriorityOrders() {
		$orders = $this->getPostVar('orders', []);
		update_option('zpm/priority/orders', $orders);
		wp_send_json_success([]);
	}
	...
	public function updateTaskStatus() {
		...
		if (!Utillities::canEditTask($taskId)) return $this->unauthorized();
	...
	public function send_comment() {
		...
		$send_email = isset($_POST['send_email']) ? zpm_sanitize_bool($_POST['send_email']) : true;
		$settings = array(
			'user_id' => $user_id,
			'subject' => $subject,
		...
		if ($attachments) {
			...
			foreach ((array) $attachments as $attachment) {
				...
				$settings['user_id'] = zpm_sanitize_int($currentUserId);
				$settings['subject'] = !empty($subject) ? $subject : sanitize_text_field($attachment['attachment_type']);
				$settings['subject_id'] = !empty($subject) ? $subject_id : sanitize_text_field($attachment['subject_id']);
				...
				$icon = isset($attachment['icon']) && !empty($attachment['icon']) ? sanitize_text_field($attachment['icon']) : '';
				...
				if (isset($attachment['filename']) && !empty($attachment['filename'])) {
					Utillities::updateFileMeta($id, 'filename', sanitize_text_field($attachment['filename']));
				}

				if ($isExternal) {
					Utillities::updateFileMeta($id, 'isExternal', zpm_sanitize_bool($isExternal));
				}
	...
	public function new_project() {
		...
		$response = [
			...
			'username'		=> $username,
			'url' 			=> $url,
			'shortcode_html' => Projects::shortcodeHtml($project)
		];
	...
	public function save_project() {
		...
		$start_date = zpm_is_date_valid($settings['date_start']) ? date_i18n($general_settings['date_format'], strtotime($settings['date_start'])) : __('Not set', 'zephyr-project-manager');
		$due_date = zpm_is_date_valid($settings['date_due']) ? date_i18n($general_settings['date_format'], strtotime($settings['date_due'])) : __('Not set', 'zephyr-project-manager');
	...
	public function update_task_priority() {
		...
		if ($task->priority !== $settings['priority']) {
			/* translators: Task priority update notification. 1: Member name, 2: Task name, 3: Priority label */
			$event_message = sprintf(esc_html('%1$s updated the priority of %2$s to %3$s', 'zephyr-project-manager'), $member['name'], esc_html($task->name), $priority_label);
		}
	...
	public function export_project() {
		...
		if (isset($_POST['export_to']) && $_POST['export_to'] == 'json') {
			...
			$name = 'Project - ' . stripslashes(esc_html($project->name)) . '.json';
			$filename = Utillities::getUploadDir($name);
			...
			$filename = Utillities::getUploadDir('Project - ' . esc_html($project->name) . '.csv');
			...
			$name = esc_html($project->name) . ' - Tasks.csv';
			$tasks_file = Utillities::getUploadDir($name);
			...
			$tasks_file = Utillities::getUploadUrl($name);
	...
	public function like_project() {
		...
		if (!Utillities::canViewProject($projectID)) $this->unauthorized();
	...
	public function copy_task() {
		...
		$newTaskID = Tasks::copy($task_id, $settings);
		$new_task = Tasks::get_task($newTaskID);
		$user_id = get_current_user_id();
		$member = Members::get_member($user_id);

		/* translators: Task copy notification. 1: Member name, 2: Task name */
		$event_message = sprintf(esc_html('%1$s copied the task %2$s', 'zephyr-project-manager'), $member['name'], esc_html($task->name));
	...
	public function export_task() {
		...
		if (isset($_POST['export_to']) && $_POST['export_to'] == 'json') {
			...
			$name = 'Task - ' . esc_html($task->name) . '.json';
			$filename = Utillities::getUploadDir($name);
			$handle = fopen($filename, 'w+');
			fwrite($handle, $formattedData);
			fclose($handle);
			...
			$name = 'Task - ' . esc_html($task->name) . '.csv';
			$filename = Utillities::getUploadDir($name);
			$filename = fopen($filename, 'w');
	...
	public function save_task() {
		...
		ob_start();
		echo apply_filters('zpm_task_update_response', esc_html($task_id));
	...
	public function filter_tasks() {
		...
		foreach ($tasks as $task) {
			if (($filter === 0 || $filter == 'my_tasks' || $assignee == $currentUserID) && $hideCompletedTasks) {
		...
		if (empty($tasks) || empty(trim($html))) {
			$html = '<p class="zpm_error_message">' . __('No results found...', 'zephyr-project-manager') . '</p>';
		}
	...
	public function get_user_by_unique_id() {
		...
		if (!Members::canViewMember($user->ID)) $this->unauthorized();
	...
	public function get_user_progress() {
		...
		<div class="zpm-member-progress__stats">
			<div class="zpm-member-progress__stat"><?php esc_html_e('Tasks', 'zephyr-project-manager'); ?>: <span class="zpm-stat-val"><?php echo esc_html($total); ?></span></div>
			<div class="zpm-member-progress__stat zpm-member-stat__completed"><?php esc_html_e('Completed Tasks', 'zephyr-project-manager'); ?>: <span class="zpm-stat-val"><?php echo esc_html(sizeof($user_completed_tasks)); ?></span></div>
			<div class="zpm-member-progress__stat zpm-member-stat__pending"><?php esc_html_e('Pending Tasks', 'zephyr-project-manager'); ?>: <span class="zpm-stat-val"><?php echo esc_html(sizeof($user_pending_tasks)); ?></span></div>
	...
	public function updateProjectView() {
		$view = isset($_POST['view']) ? zpm_sanitize_int($_POST['view']) : '';
		update_user_meta(get_current_user_id(), 'project_view', $view);
		echo json_encode([]);
		die();
	}
...
<?php
	...
	public function getCalendarItems() {
		...
		foreach ($tasks as $key => $task) {
			...
			$task->formattedStart = $isStartValid ? zpm_date($task->date_start, '', 'Y-m-d', true) : '';
			$task->subtasks = Tasks::get_subtasks($task->id);
			$startTime = explode(' ', $task->date_start);
			$dueTime = explode(' ', $task->date_due);
			$task->isAllDay = true;

			if (isset($startTime[1])) {
				if ($startTime[1] !== '00:00') {
					$task->time = $startTime[1];
					$task->isAllDay = false;
				}
			}
			...
			foreach ($task->subtasks as $k => $subtask) {
				$task->subtasks[$k]->url = Utillities::getURL('task', [
					'id' => $subtask->id
				], isset($_POST['frontend']) ? $_POST['frontend'] : '');
			}
			...
			$task->formattedDue = $isDueValid ? zpm_date($task->date_due, '', 'Y-m-d', true) : '';
	...
	public function sendEmail() {
		...
		if (!empty($emails)) {
			foreach ($emails as $email) {
				Emails::send_email($email, $subject, $html, 'email');
			}
		} else {
			$member = Members::get_member($userId);
			Emails::send_email($member['email'], $subject, $html, 'email');
		}
	...
	public function exportTasksToCSV() {
		...
		header('Expires: 0');
		$tasks = Tasks::getAllTasks();
		$csvName = 'ZPM Tasks.csv';
		$filename = Utillities::getUploadDir($csvName);
		$filename = fopen($filename, 'w+');
		...
		$filename = Utillities::getUploadUrl($csvName);
		echo json_encode($filename);
		wp_die();
	}
	...
	public function filter_tasks_by() {
		...
		$current_filter = $_POST['current_filter'] ?? '-1';
		$assignee = $_POST['assignee'] ?? '-1';
		$user_id = get_current_user_id();
		$tasks = array();
		$taskRowFilters = [];
