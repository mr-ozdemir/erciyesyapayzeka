...
function zpm_daily_scheduled_tasks() {
	...
	$dayBefore = zpm_get_datetime('-2 days')->getTimestamp();
	$tomorrow = zpm_get_datetime('+1 days');
	$tomorrowFormat = $tomorrow->format('Y-m-d');
	$todayFormat = zpm_get_datetime()->format('Y-m-d');
	...
	foreach ($tasks as $task) {
		...
		if ($dueDate !== '0000-00-00 00:00:00') {
			...
			if (Tasks::isOverdue($task) && $dueDate > $dayBefore) {
				// Send overdue email
				if (!Tasks::isCompleted($task)) {
					Emails::sendTaskOverdueEmail($task);
				}
			} else {
				$dueDateFormat = date('Y-m-d', $dueDate);

				if ($dueDate == $tomorrowFormat) {
					// Send task due tomorrow email
					if (!Tasks::isCompleted($task)) {
						Emails::sendTaskDueEmail($task);
					}
				}
			}
...
function zpm_date_i18n($date, $default = 'none', $format = '', $includeTime = false) {
	global $zpmSettings;
	...
	if (strpos($dateIl8n, '00:00') !== false) {
		$dateIl8n = str_replace(' 00:00', '', $dateIl8n);
	}
