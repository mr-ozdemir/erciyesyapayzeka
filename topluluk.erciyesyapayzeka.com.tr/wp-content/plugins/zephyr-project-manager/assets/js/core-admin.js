...
function zpmAddFilter(hookName, callback) {
	if (!zpmFilters[hookName]) {
		zpmFilters[hookName] = [];
	}
	zpmFilters[hookName].push(callback);
}

// Function to apply filters
function zpmApplyFilter(hookName, value) {
	if (zpmFilters[hookName]) {
		zpmFilters[hookName].forEach(callback => {
...
jQuery(document).ready(function ($) {
	...
	var $zpmMemberList = $('#zpm-member-list__table');
	if ($zpmMemberList.length > 0) {
		$zpmMemberList.DataTable();
	}
	...
	$('#zpm_import_tasks_from_json').on('click', function () {
		...
		jsonFileUploader = wp.media.frames.file_frame = wp.media(args);
	...
	$('body').on('click', '.zpm_project_title', function (e) {
		if (!zpm_localized.settings.direct_link_project) {
			...
			ZephyrProjects.ajax(data, function (response) {
				...
				$('body').find('.zpm_quicktask_content #zpm_quicktask_date').datepicker({ dateFormat: ZephyrProjects.getDateFormat() });
	...
	$('body').on('click', '#zpm-new-task__new-file', function () {
		...
		zpm_new_task_uploader = wp.media.frames.file_frame = wp.media(args);
	...
	let createTaskValid = zpmApplyFilter('zpm/tasks/create', true);
	console.log('create task valid', createTaskValid);

	if (!createTaskValid) return;
		...
		var data = {
			...
			task_due_date: ZephyrProjects.getDate(due_date),
			task_start_date: ZephyrProjects.getDate(start_date),
		...
		if (zpm_localized.current_project !== '') {
			jQuery('body').find('#zpm_new_task_project').val(zpm_localized.current_project).trigger('chosen:updated');
		}
	...
	$('body').on('click', '.zpm_task_list_row', function (e) {
		...
		if ($target.hasClass('zpm_task_project') || $target.hasClass('zpm-override-click') || $target.closest('.zpm-override-click').length > 0) {
			e.preventDefault();
			e.stopPropagation();
			console.log('project');
			return;
	...
	$('body').on('click', '#zpm_create_quicktask', function () {
		...
		var data = {
			...
			task_due_date: ZephyrProjects.getDate(taskDueDate.val())
		};

		ZephyrProjects.create_task(data, function (response) {
			$('body').find('.zpm_task_list').append(response.new_task_html);
			...
			zpmSortTasksByDueDate('#zpm-project-task-list');
		});
	});
	...
	$('body').on('click', '#zpm_copy_task_btn', function () {
		...
		ZephyrProjects.copy_task(data, function (response) {
			$('body').find('.zpm_task_list').append(response.html);
			$('body').find('.zpm_message_center').remove();
		});
	...
	function zpm_import_tasks(attachment) {
		if (attachment.mime == 'text/csv') {
			...
			ZephyrProjects.upload_tasks(data, function (response) {
				...
				$('.zpm_task_list').append(response.html);
			});
	...
	$('body').on('click', '#zpm_project_save_settings', function () {
		...
		$('#zpm_edit_project_description').mentionsInput('val', function (text) {
			...
			var data = {
				...
				project_name: name,
				project_description: description,
				project_due_date: ZephyrProjects.getDate(due_date),
				project_start_date: ZephyrProjects.getDate(start_date),
	...
	$('body').on('click', '.zpm-edit-message', function () {
		var comment = $(this).closest('.zpm_comment');
		var comment_id = comment.data('zpm-comment-id');
		var message = comment.find('.zpm_comment_content').html();
		console.log('message', message);
		var html = '<input type="hidden" data-ajax-name="message-id" value="' + comment_id + '" />' +
		'<div class="zpm-contenteditable" contenteditable="true" type="text" data-ajax-name="message">' + message + '</div>';
		ZephyrProjects.zephyrModal(zpm_localized.strings.edit_message, html, zpm_localized.strings.save_changes, function (modal) {
			...
			var message = modal.find('[data-ajax-name="message"]').html();

			comment.find('.zpm_comment_content').html(message);
			ZephyrProjects.ajax({
	...
	$('body').on('change', '.zpm_subtask_is_done', function () {
		var task_id = $(this).data('task-id');
		var isChecked = $(this).is(':checked');

		console.log('ischecked', isChecked, $(this));
	...
	function zpmSetupRippleEffect() {
		...
		$('body').on('click', '.zpm_button,[data-ripple], [ripple], [zpm-ripple], .zpm_project_title.project_name, .zpm_button_outline', function (e) {
			...
			if ($self.attr('disabled') || e.target.className.indexOf('zpm_task_mark_complete') > -1 || e.target.className.indexOf('zpm-material-checkbox-label') > -1 || e.target.className.indexOf('zpm-task-bulk-selector') > -1 || e.target.className.indexOf('zpm_toggle') > -1 || e.target.className.indexOf('zpm_task_project') > -1 || e.target.className.indexOf('zpm-override-click') > -1) {
				return;
			}
	...
	if (zephyrSocket) {
		zephyrSocket.on('task-created', function (data) {
			if (typeof data.user_id !== zpm_localized.user_id) {
				...
				$('body').find('.zpm_task_list').append(new_task);
				$('body').find('.zpm_message_center').remove();
				$('body').find('#zpm_task_option_container').removeClass('zpm_hidden');
				$('body').find('#zpm_task_list_container').removeClass('zpm_hidden');
				$('body').find('.zpm_no_results_message').addClass('zpm_hidden');
	...
	$('.zpm-project-view__option').on('click', function () {
		...
		ZephyrProjects.updateProjectView(view);
