...
<div id="zpm_create_task" class="zpm-modal <?php echo esc_attr($extra_classes); ?> zpm-form" role="dialog" aria-labelledby="zpm_task_add_new" aria-modal="true" aria-hidden="true">
	...
	<div class="zpm_modal_body" role="form" aria-label="<?php echo !$isScrum ? __('New Task', 'zephyr-project-manager') : __('New User Story', 'zephyr-project-manager'); ?>">
		<div class="zpm_modal_content">
			...
			<?php if (!$isProject) : ?>
				<div class="zpm-new-task-field__project">
					...
					<select id="zpm_new_task_project">
						<option value="-1" <?php echo $defaultProject == '-1' ? 'selected' : ''; ?>><?php esc_html_e('Select Project', 'zephyr-project-manager'); ?></option>
			...
			<?php else : ?>
				<input type="hidden" id="zpm_new_task_project" value="<?php echo esc_attr($projectID) ?>" />
			<?php endif; ?>

			<div class="zpm-new-task-field__assignee">
				<label class="zpm_label" for="zpm_new_task_assignee"><?php esc_html_e('Assignee', 'zephyr-project-manager'); ?></label>
				<select id="zpm_new_task_assignee" multiple data-placeholder="<?php esc_html_e('Select Assignees', 'zephyr-project-manager'); ?>" data-default="<?php echo $defaultAssignee; ?>">
			...
			<div class="zpm-new-task-field__team">
				<label class="zpm_label" for="zpm-new-task-team-selection"><?php esc_html_e('Team', 'zephyr-project-manager'); ?></label>
			...
			<div class="zpm_options_container zpm-new-task-field__dates">
				<span class="zpm_options_col zpm-new-task-field__start-date">
					<div class="zpm-form__group">
						<input type="text" autocomplete="off" name="zpm_new_task_start_date" id="zpm_new_task_start_date" class="zpm-form__field" data-default="<?php echo $defaultStart; ?>" value="<?php echo $defaultStart; ?>" placeholder="<?php esc_html_e('Start Date', 'zephyr-project-manager'); ?>" data-start="<?php echo $isProject ? date('Y-m-d', strtotime($project->date_start)) : ''; ?>">
						<label for="zpm_new_task_start_date" class="zpm-form__label"><?php esc_html_e('Start Date', 'zephyr-project-manager'); ?></label>
					</div>
				</span>

				<span class="zpm_options_col zpm-new-task-field__due-date">
					<div class="zpm-form__group">
						<input type="text" autocomplete="off" name="zpm_new_task_due_date" id="zpm_new_task_due_date" class="zpm-form__field" data-default="<?php echo $defaultDue; ?>" value="<?php echo $defaultDue; ?>" placeholder="<?php esc_html_e('Due Date', 'zephyr-project-manager'); ?>">
						<label for="zpm_new_task_due_date" class="zpm-form__label"><?php esc_html_e('Due Date', 'zephyr-project-manager'); ?></label>
					</div>
				</span>
			...
			<div class="zpm-new-task-field__status">
				<label class="zpm_label" for="zpm-new-task__status"><?php esc_html_e('Status', 'zephyr-project-manager'); ?></label>
				<select id="zpm-new-task__status" data-position="top" class="zpm_input zpm-input-chosen" data-default="not_started">
					...
					<?php foreach ($statuses as $slug => $status) : ?>
						<option value="<?php echo esc_attr($slug); ?>" <?php echo $slug == Utillities::getSetting('default_status') ? 'selected' : ''; ?>><?php echo esc_html($status['name']); ?></option>
					<?php endforeach; ?>
				</select>
			</div>
			...
			<div class="zpm-new-task-task--categories">
				<label class="zpm_label" for="zpm-new-task--categories"><?php esc_html_e('Category', 'zephyr-project-manager'); ?></label>
			...
			<div class="zpm-new-task--parent-task">
				...
				<select class="zpm_input zpm-input-chosen zpm-chosen" data-ajax-name="parent-id" data-default="-1" data-placeholder="<?php esc_html_e('Select Task', 'zephyr-project-manager') ?>">
					...
					<?php foreach ($tasks as $taskItem) : ?>
						...
						<option value="<?php echo esc_attr($taskItem->id); ?>"><?php echo esc_html($taskItem->name); ?></option>
					<?php endforeach; ?>
				</select>
			</div>
			...
			<div id="zpm-new-task__recurrence" class="zpm-task-recurrence__wrap">
				<div id="zpm-new-task__recurrence-selection" class="zpm-recurrence-selection">
					...
					<select id="zpm-new-task__recurrence-select" class="zpm-chosen-select zpm_input zpm-input-chosen" data-position="top">
						<option value="default" selected><?php esc_html_e('None', 'zephyr-project-manager'); ?></option>
						<option value="daily"><?php esc_html_e('Daily', 'zephyr-project-manager'); ?></option>
						<option value="weekly"><?php esc_html_e('Weekly', 'zephyr-project-manager'); ?></option>
						<option value="monthly"><?php esc_html_e('Monthly', 'zephyr-project-manager'); ?></option>
						<option value="annually"><?php esc_html_e('Annually', 'zephyr-project-manager'); ?></option>
					</select>
					...
					<div class="zpm-new-task__recurrence-section" data-section="daily" style="display: none;">
						...
						<div class="zpm-new-task__recurrence-settings">
							<select id="zpm-new-task__recurrence-daily" class="zpm-multi-select" multiple data-placeholder="<?php esc_html_e('Days to Repeat', 'zephyr-project-manager'); ?>">
								<option value="0" selected><?php esc_html_e('Monday', 'zephyr-project-manager'); ?></option>
								<option value="1" selected><?php esc_html_e('Tuesday', 'zephyr-project-manager'); ?></option>
								<option value="2" selected><?php esc_html_e('Wednesday', 'zephyr-project-manager'); ?></option>
								<option value="3" selected><?php esc_html_e('Thursday', 'zephyr-project-manager'); ?></option>
								<option value="4" selected><?php esc_html_e('Friday', 'zephyr-project-manager'); ?></option>
								<option value="5"><?php esc_html_e('Saturday', 'zephyr-project-manager'); ?></option>
								<option value="6"><?php esc_html_e('Sunday', 'zephyr-project-manager'); ?></option>
							</select>
						</div>
		...
		<div class="zpm_modal_buttons">
			...
			<span id="zpm-new-task-priority" class="zpm_button zpm_button_secondary" zpm-toggle-dropdown="zpm-new-task-priority-dropdown" data-priority="priority_none"><span class="zpm-priority-name"><?php esc_html_e('Set Priority', 'zephyr-project-manager'); ?></span>
				<div id="zpm-new-task-priority-dropdown" class="zpm-dropdown">

					<div class="zpm-dropdown-item zpm-new-task-priority" data-value="priority_none" data-color="#f9f9f9"><span class="zpm-priority-indicator zpm-color-none"></span><?php esc_html_e('None', 'zephyr-project-manager'); ?></div>
			...
			<?php if (Utillities::canUploadFiles()): ?>
				<button id="zpm-new-task__new-file" class="zpm_button"><?php esc_html_e('Add File', 'zephyr-project-manager'); ?></button>
			<?php endif; ?>
