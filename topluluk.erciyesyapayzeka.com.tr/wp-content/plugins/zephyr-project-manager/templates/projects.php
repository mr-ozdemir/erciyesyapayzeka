	...
	<div id="zpm_container" class="zpm_add_project">
		<div class="zpm_body" style="display: none;">
			<ul class="nav nav-tabs">
				<li class="active"><a href="#tab-1"><?php esc_html_e('New Project', 'zephyr-project-manager'); ?></a></li>
			</ul>
		</div>
			...
			<?php if ($show_projects) : ?>
				...
				<?php if (!empty($category_id) || $settings['enable_category_grouping'] == false) : ?>
					...
					<div id="zpm-projects__view-options">
						<span id="zpm-project-view__archived" class="zpm-button__block zpm-fa-icon zpm-toggle-state zpm-color__hover-primary fa fa-archive" title="<?php esc_html_e('View Archived Projects', 'zephyr-project-manager'); ?>"></span>
						<span class="zpm-button__block zpm-project-view__option zpm-color__hover-primary fa fa-th-large <?php echo $list_view ? '' : 'zpm-state__active'; ?>" data-view="grid" title="<?php esc_html_e('Grid', 'zephyr-project-manager'); ?>"></span>
						<span class="zpm-button__block zpm-project-view__option zpm-color__hover-primary fa fa-th-list <?php echo $list_view ? 'zpm-state__active' : ''; ?>" data-view="list" title="<?php esc_html_e('List', 'zephyr-project-manager'); ?>"></span>
						<span id="zpm-project-view__title"><?php esc_html_e('All Projects', 'zephyr-project-manager'); ?></span>
					</div>

					<div id="zpm-projects__filters">
						<select data-zpm-project-sorting>
							<?php foreach ($sortingMethods as $key => $sortingMethod) : ?>
								<option value="<?php echo esc_attr($key); ?>" <?php selected($key, $lastSortingMethod) ?>><?php echo esc_html($sortingMethod) ?></option>
							<?php endforeach; ?>
						</select>
					</div>
					...
					<div id="zpm_projects_holder" class="zpm_body">
						<div id="zpm_project_manager_display" class="<?php echo ($project_count == '0') ? 'zpm_hide' : ''; ?>">
							<div id="zpm_project_page_options">
								<?php if (Utillities::can_create_projects()) : ?>
									<button id="zpm_create_new_project" class="zpm_button"><?php esc_html_e('New Project', 'zephyr-project-manager'); ?></button>
								<?php endif; ?>
							</div>
						</div>
						...
						<?php if ($project_count == '0') : ?>
							<div class="zpm_no_results_message">
								<?php if (Utillities::can_create_projects()) : ?>
									<?php /* translators: Message prompting the user to take action. %1$s: Opening anchor tag, %2$s: Closing anchor tag */ ?>
									<?php printf(esc_html('No projects created yet. To create a project, click on the \'Add\' button at the top right of the screen or click %1$s here %2$s', 'zephyr-project-manager'), '<a id="zpm_first_project" class="zpm_button_link">', '</a>'); ?>
								<?php else : ?>
									<?php esc_html_e('No projects created yet.', 'zephyr-project-manager'); ?>
								<?php endif; ?>
							</div>
						<?php endif; ?>
				...
				<?php else : ?>
					...
					<h4 id="zpm-header-breadcrumb">
						...
						<?php esc_html_e('Choose a category', 'zephyr-project-manager'); ?>
					</h4>
					<div class="zpm-grid zpm-category-grid">
						<div class="zpm-grid__row">
							...
							<?php if ($categoryCount == 0) : ?>
								<div class="zpm_no_results_message"><?php esc_html_e('No categories available.', 'zephyr-project-manager'); ?></div>
							<?php endif; ?>
						</div>
					</div>
				<?php endif; ?>
