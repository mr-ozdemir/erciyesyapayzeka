<?php
	...
	if ( in_array('administrator', (array) $u->roles) && (int)$currentUserId !== (int)$user_id ) {
		?>
		<p id="zpm-error__no-edit-access" class="zpm-panel"><?php esc_html_e( "Can't edit administrators", "zephyr-project-manager" ); ?></p>
		<?php
		exit;
	}
	...
	if (isset($_POST['zpm_profile_settings'])) {
		...
		// $role = isset($_POST['zpm-user-settings__role']) ? sanitize_text_field($_POST['zpm-user-settings__role']) : false;
		$roles = isset($_POST['zpm-user-settings__role']) ? $_POST['zpm-user-settings__role'] : [];
		$data = ['ID' => $user_id, 'role' => $roles];

		if ( !empty($roles) && $admin ) {
			foreach ($u->roles as $currentRole) {
				if (!in_array($currentRole, $roles)) {
					$u->remove_role($currentRole);
				}
			}
		...
		$settings = apply_filters('zpm/user/data', $settings, $user_id);
	...
	$user_settings_option = apply_filters('zpm/user/data', $user_settings_option, $user_id);
	...
	$url = $admin ? admin_url('/admin.php?page=zephyr_project_manager_teams_members') : Utillities::get_frontend_url('action=users');
	...
	<h1 class="zpm_page_title">
		<a class="zpm-back-link" href="<?php echo esc_url($url); ?>"><i class="lnr lnr-chevron-left"></i></a> <?php esc_html_e( 'Edit Member', 'zephyr-project-manager' ); ?> - <?php echo esc_html($settings_name); ?></h1>
		<!-- Profile Settings -->
		<div id="zpm-edit-member-container" class="zpm_body">
			<form id="zpm_profile_settings" method="post">
				...
				<input type="hidden" id="zpm_gravatar" value="<?php echo esc_attr(get_avatar_url($user_id)); ?>" />

				<div class="zpm-form__group">
					<input type="text" name="zpm_settings_name" id="zpm_settings_name" class="zpm-form__field" placeholder="<?php esc_html_e( 'Name', 'zephyr-project-manager' ); ?>" value="<?php echo esc_html($settings_name); ?>">
				...
				<?php if ( $admin ) : ?>
					<div class="zpm-form__group">
						<input type="text" name="zpm-settings-unique-id" id="zpm-settings-unique-id" class="zpm-form__field" placeholder="<?php esc_html_e( 'Unique ID', 'zephyr-project-manager' ); ?>" value="<?php echo esc_attr($unique_id); ?>">
						<label for="zpm-settings-unique-id" class="zpm-form__label"><?php esc_html_e( 'Unique ID', 'zephyr-project-manager' ); ?></label>
					</div>
				<?php endif; ?>
				...
				<label for="zpm_notify_tasks" class="zpm-material-checkbox">
					...
					<span class="zpm-material-checkbox-label"><?php esc_html_e( 'Task Reminders', 'zephyr-project-manager' ); ?></span>
				</label>
				...
				<?php if ($admin && intval($user_id) !== intval(get_current_user_id())): ?>
					<label class="zpm_label"><?php esc_html_e( 'Role', 'zephyr-project-manager' ); ?></label>
					<select id="zpm-user-settings__role" name="zpm-user-settings__role[]" multiple class="zpm_select zpm-multi-select zpm-select">
						<?php foreach ($wp_roles as $slug => $role) : ?>
							<?php if (isset($role['name'])) : ?>
								<option value="<?php echo esc_attr($slug); ?>" <?php echo in_array( $slug, (array) $u->roles ) ? 'selected' : ''; ?>><?php echo esc_html($role['name']); ?></option>
							<?php endif; ?>
						<?php endforeach; ?>
					</select>

					<label class="zpm_label"><?php esc_html_e( 'Allow access to Zephyr', 'zephyr-project-manager' ); ?></label>
					<label for="zpm-can-zephyr-<?php echo esc_attr($user_id); ?>" class="zpm-material-checkbox">
						<input type="checkbox" id="zpm-can-zephyr-<?php echo esc_attr($user_id); ?>" name="zpm_can_zephyr" class="zpm_toggle invisible" data-user-id="<?php echo esc_attr($user_id); ?>" <?php echo $can_zephyr == "true" ? 'checked' : ''; ?> value="1">
						<span class="zpm-material-checkbox-label"><?php esc_html_e( 'Allow Access', 'zephyr-project-manager' ); ?></span>
					</label>
