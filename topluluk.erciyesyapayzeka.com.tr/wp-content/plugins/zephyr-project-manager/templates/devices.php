...
<div class="zpm_settings_wrap">
	...
	<div id="zpm_container" class="zpm_custom_fields">
		<h3 class="zpm-info-title"><?php esc_html_e( 'Linked Devices', 'zephyr-project-manager' ); ?> <small class="zpm-heading-subtext">- <?php esc_html_e( 'Displays all your linked mobile devices using the Zephyr Project Manager mobile app', 'zephyr-project-manager' ); ?></small></h3>
		
		<?php if (sizeof( $devices ) <= 0) : ?>
			<p class="zpm-no-devices-notice">
				<?php /* translators: Message prompting the user to download the Android app from the Play Store. %1$s: Opening anchor tag with app URL, %2$s: Closing anchor tag */ ?>
				<?php printf( __( 'There are no devices linked to your website. You can download the Android app from the Play Store %1$s here to start managing your projects from your device and get more done. %2$s', 'zephyr-project-manager' ), '<a target="_blank" href="https://play.google.com/store/apps/details?id=com.zephyr.dylank.zephyrprojectmanager">', '</a>' ) ?></p>
		<?php endif; ?>
				...
				<?php foreach ( $devices as $device ) : ?>
					...
					<div class="zpm-linked-device">
						<div class="zpm-linked-device-card">
							<h3 class="zpm-device-name"><?php echo esc_html($device['name']); ?></h3>
							<h4 class="zpm-device-id"><?php echo esc_html($device['id']); ?></h4>
							<div class="zpm-device-user">
								<span class="zpm-device-user-name"><?php echo esc_html($linked_user['name']); ?></span>
							</div>
						</div>
					</div>
				<?php endforeach; ?>
