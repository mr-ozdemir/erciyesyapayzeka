...
<main class="zpm_settings_wrap">
	...
	<div id="zpm_container">
		...
		<article class="zpm_body">
			<h3><?php esc_html_e( 'Calendar', 'zephyr-project-manager' ); ?></h3>
				...
				<div class="zpm-calendar__filter-section">
					...
					<select id="zpm-calendar__filter-completed">
						...
						<?php foreach ($statuses as $slug => $status): ?>
							<option value="<?php echo in_array($slug, ['completed', 'complete']) ? '1' : $slug ?>"><?php echo esc_html($status['name']); ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			</div>
			...
			<?php if (Utillities::getSetting('ics_sync_enabled')): ?>
				<div id="zpm-calendar-syncing-container">
					<label for=""><?php esc_html_e('ICS File', 'zephyr-project-manager'); ?>: </label><a class="zpm-ics-link" href="<?php echo esc_attr($icsUrl); ?>"><?php echo esc_html($icsUrl); ?></a>
				</div>
			<?php endif; ?>
		</article>
	</div>
